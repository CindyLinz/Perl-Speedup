...
END
    ...
    my %boost;
    if( $o->{ILSM}{code} =~ /\S/ ) {
        %boost = map { $_ => 1 } $o->{ILSM}{code} =~ /(\S+)/g;
    }

    for my $thing (sort keys %{$pkg.'::'}) {
        if( defined &{$pkg.'::'.$thing} && (!keys %boost || $boost{$thing}) ) {
            print "  $pkg\::$thing\n";
            my $op_chain = Speedup::op_chain(\&{$pkg.'::'.$thing});
            print Dumper($op_chain),$/;
            if( $op_chain ) {
                $XS .= Speedup::gen_code($thing, $op_chain);
            }
        }
    }
